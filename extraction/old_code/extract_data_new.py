...
FILES_TO_REMOVE = ['00099_00634_20051125-GBNNN-VDNN-15.C3D',
                   ...
                   '00478_00867_20060512-GBNNN-VDNN-12.C3D',
                   '00155_04095_20160426-GBNTN-VDEN-07.C3D',
...
class Extracter:
    def __init__(self, datasets, keep_pathology=['CP_Spastic_Uni_Hemiplegia']):
        ...
        print('We have found {} files and keep {} !'.format(self.n_total_files,
                                                            len(self.files)))

    def _keep_file(self, dataset, file_name):
        # check if it's a file
        is_file = os.path.isfile(join_path(dataset, file_name))
        # check if it's a c3d
        is_c3d = (file_name[-3:].lower() == 'c3d')

        # check if the pathology is what we need
        if dataset == 'DATASETS/Healthy':
            keep_patho = True
        else:
            keep_patho = (self.get_diagnostic(join_path(dataset, file_name))
                          in self.keep_pathology)

        # check if file isn't in remove files
        is_to_remove = (file_name not in FILES_TO_REMOVE)
        # check end
        if dataset == 'DATASETS/CP_Gait_1.0':
    ...
    def _get_files(self):
        ...
        for dataset in self.datasets:
            for f_name in os.listdir(dataset):
                self.n_total_files += 1
                print("SEARCHING files ...", end='\r')
                if self._keep_file(dataset, f_name):
                    files.append(join_path(dataset, f_name))
    ...
    def extract(self, examination=False, diagnostic=False, affected_side=False, gmfcs=False, markers=False, angles=False, events=False,
                all_=False):
        ...
        for file_id in range(len(self.files)):
            ...
            if diagnostic or all_:
                if dataset == 'Healthy':
                    ...
                    self.data['diagnostic'][-1] = self.get_diagnostic(self.files[file_id])
            if affected_side or all_:
                if dataset == 'Healthy':
                    self.data['affected_side'][-1] = 'normal'
                else:
                    self.data['affected_side'][-1] = self.get_affected_side(self.files[file_id])
            if gmfcs or all_:
    ...
    def get_examination(self, file_path):
        ...
        for child_id in range(1, n_childs):
            ...
            if label.lower() in self.list_labels:
                value = meta_subject.FindChild(label).value().GetInfo().ToString()[0].replace(' ', '')
    ...
    def get_diagnostic(self, file_path):
        self._update_btk(file_path)
        diag = self.metadata_reader.FindChild("SUBJECTS").value().FindChild("DIAGNOSTIC").value().GetInfo().ToString()[
            0]
        return diag
    ...
    def get_affected_side(self, file_path):
        self._update_btk(file_path)
        aff_side = \
        self.metadata_reader.FindChild("SUBJECTS").value().FindChild("AFFECTED_SIDE").value().GetInfo().ToString()[0]
    ...
    def get_gmfcs(self, file_path):
        self._update_btk(file_path)
        gmfcs = self.metadata_reader.FindChild("SUBJECTS").value().FindChild("GMFCS").value().GetInfo().ToString()[0]
        return gmfcs
    ...
    def get_labels_examination(self):
        ...
        for file_id in range(len(self.files)):
            print("SEARCHING FEATURES FOR EXAMINATION IN FILE {} OVER {}".format(file_id, len(self.files)), end='\r')
    ...
    def get_labels_angles(self):
        ...
        files_ = self.files[:]
        for file_id in range(len(self.files)):
            print("SEARCHING NAMES FOR ANGLES IN FILE {} OVER {}".format(file_id, len(self.files)), end='\r')
            reader = btk.btkAcquisitionFileReader()
            reader.SetFilename(self.files[file_id])
            reader.Update()
            acq = reader.GetOutput()
            metadata = acq.GetMetaData()
            labels_list_tmp = list(metadata.FindChild("POINT").value().FindChild("ANGLES").value().GetInfo().ToString())
    ...
    def get_angles(self, file_path):
        ...
        angles_data = [self.btk_reader.GetOutput().GetPoint(angle).GetValues() for angle in self.angles_names]
    ...
    def get_events(self, file_path):
        ...
        event_side = list(
            self.metadata_reader.FindChild("EVENT").value().FindChild("CONTEXTS").value().GetInfo().ToString())
        event_side = [name.replace(' ', '') for name in event_side]
        event_type = list(
            self.metadata_reader.FindChild("EVENT").value().FindChild("LABELS").value().GetInfo().ToString())
        event_type = [name.replace(' ', '') for name in event_type]
        event_frames, event_side, event_type = zip(*sorted(zip(event_frames, event_side, event_type)))

        events_data = []
        events_data.append(['frames'] + (np.array(event_frames) - acq.GetFirstFrame()).tolist())
    ...
    def save(self, output_folder='data_extracted'):
        ...
        for key in self.data.keys():
            ...
            if key == 'angles' or key == 'markers':
                ...
                for idx in range(len(self.data['angles'])):
                    np.save(join_path(folder_tmp, self.data['files'][idx].split('.')[0]), self.data['angles'][idx])
            elif key == 'events':
                folder_tmp = join_path(output_folder, 'events')
                if not os.path.exists(folder_tmp):
                    os.makedirs(folder_tmp)
                for idx in range(len(self.data['events'])):
                    save_csv(join_path(folder_tmp, self.data['files'][idx].split('.')[0]), self.data['events'][idx][1:],
...
if __name__ == "__main__":
    datasets = ['DATASETS/CP_Gait_1.0',
                'DATASETS/ITW_RETRACTION_SOL_TRICEPS_Gait_1.0',
                'DATASETS/ITW_RETRACTION_TRICEPS_Gait_1.0',
                'DATASETS/Healthy']
    ...
    print('Setting angles names\n')
    extracter.angles_names = ['RAnkleAngles', 'LAnkleAngles', 'RHipAngles', 'LHipAngles', 'RPelvisAngles',
                              'LKneeAngles', 'RKneeAngles', 'LPelvisAngles']
    ...
    print('Extracting\n')
    extracter.extract(examination=False, diagnostic=True, affected_side=True, markers=True, gmfcs=False, angles=True, events=True,
                      all_=False)
    print('Saving\n')
    extracter.save(output_folder='temp_folders/data_extracted_pablo')
